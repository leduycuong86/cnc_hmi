...
double DataSource::loadFromExcel(int rowCount, int colCount)
{
    double rc = 0;
    // Append the new data depending on the type
    Document xlsxR("monthly.xlsx");
    if ( xlsxR.load() ) // load excel file
    {
        Cell* cell = xlsxR.cellAt(rowCount, colCount); // get cell pointer.
        if ( cell != NULL )
        {
            QVariant var = cell->readValue(); // read cell value (number(double), QDateTime, QString ...)
            rc = var.toDouble();
            //qDebug() << "[debug][error] cell(" << rowCount << "," << colCount <<") = " << rc;
        }
        else
        {
